...
export const handleLogin = async (req, res) => {
  ...
  if (authProvider === 'local') {
    ...
    let foundUser;
    console.log(emaillOrPhone,pwd );
    
    if (!isPhoneNumber) {
      foundUser = await User.findOne({ email: emaillOrPhone }).exec();
    } else {
      foundUser = await User.findOne({ phone: emaillOrPhone }).exec();
      console.log('funderuser, ', foundUser);
